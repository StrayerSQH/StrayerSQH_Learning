...
public:
    ...
    void ChangeCourse(CourseInformation* front, CourseInformation* change, CourseInformation* back)
    {
        ...
        {
            front->termnext = change;
            change->termnext = back;
        }
    }
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        while (qListCurrent != NULL)
        {
            ...
            while (tmp != NULL)
            {
                ...
                if (courseMap[tmp->startTime - 1][tmp->day - 1] == 0 && courseMap[tmp->endTime - 1][tmp->day - 1] == 0 && rewritePermission)
                {
            ...
            else //表明没法在现有课表情况下顺利置入，则需要返回对上一步中的必修课时间进行调整
            {
                ...
                storeCurrentChild.ChangeCourse(storeCurrentChild.GetCourseInNumber(returntoNum), eachCourseInformationGeneralList.GetCourseInformationInGeneralList(qListCurrent->courseTypeId)->childnext, storeCurrentChild.GetCourseInNumber(returntoNum + 2));
            }
